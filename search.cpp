...
void a_star(Graph graph, Location start, Location goal, unordered_map<Location, Location>& came_from, unordered_map<Location, double>& cost_so_far) {
	...
	while (!frontier.empty()) {
		Location curr = frontier.get();

		if (curr == goal) break;
		for (Location next : graph.neighbours(curr)) {
			double new_cost = cost_so_far[curr] + graph.cost(curr, next);
